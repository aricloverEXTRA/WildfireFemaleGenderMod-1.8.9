...
public class WardrobeBrowserScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        try { holidayEnabled = Configuration.getHolidayThemes(_p); } catch (Throwable ignored) {}
        ...
        this.selectedGender = Configuration.getGender(player);
        this.isDarkMode = Configuration.getDarkMode(player);
        ...
        switch (button.id) {
            case 0:
                if ("Male".equals(this.selectedGender)) {
                    ...
                    Configuration.setGender(player, "Female");
                    Configuration.getPlayerSettings(player).breastsEnabled = true;
                } else if ("Female".equals(this.selectedGender)) {
                    this.selectedGender = "Other";
                    Configuration.setGender(player, "Other");
                    Configuration.getPlayerSettings(player).breastsEnabled = true;
                } else {
                    this.selectedGender = "Male";
                    Configuration.setGender(player, "Male");
                    Configuration.getPlayerSettings(player).breastsEnabled = false;
                }
                button.displayString = getColoredGenderText(this.selectedGender);
                this.initGui();
                break;
            case 1:
                this.mc.displayGuiScreen(new WildfireBreastCustomizationScreen());
                break;
            case 3:
                isDarkMode = !isDarkMode;
                Configuration.setDarkMode(player, isDarkMode);
                this.initGui();
                break;
            case 4:
