...
public class GenderLayer implements LayerRenderer<AbstractClientPlayer> {
    ...
    public void doRenderLayer(AbstractClientPlayer player, float limbSwing, float limbSwingAmount,
        ...
        Configuration.PlayerGenderSettings cfg = isFake ? Configuration.getStaticFakeCreditsSettings()
                : Configuration.getPlayerSettings((EntityPlayer) player);
    ...
    private void renderBox(AbstractClientPlayer player, UVLayout layout, float x, float y, float z, int dx, int dy, int dz, float delta, boolean isOverlay, float renderScale, boolean useArmor, ItemStack chest, boolean isLeft) {
        ...
        if (useArmor && chest != null) {
            ...
            String texPath = ia.getArmorTexture(chest, player, 2, isOverlay ? "overlay" : null);
            if (texPath == null && isOverlay) texPath = ia.getArmorTexture(chest, player, 2, null);
            if (texPath != null) {
                tex = new ResourceLocation(texPath);
    ...
    private void renderBoxWithUVs(AbstractClientPlayer player, UVLayout layout, float x, float y, float z, int dx, int dy, int dz, float delta, boolean isOverlay, float renderScale, boolean useArmor, ItemStack chestStack, boolean isLeft) {
        ...
        if (hasChestplate) {
            ...
            if (chest != null && chest.getItem() instanceof ItemArmor) {
                ...
                String texPath = ia.getArmorTexture(chest, player, 2, isOverlay ? "overlay" : null);
                if (texPath == null && isOverlay) texPath = ia.getArmorTexture(chest, player, 2, null);
                if (texPath != null) armorTex = new ResourceLocation(texPath);
            }
        }
        ResourceLocation tex;
        ...
        x2 += d; y2 += d; z2 += d;
        this.renderPlayer.bindTexture(tex);
        if (useArmorTex && chestStack != null) {
            boolean isLeather = chestStack.getItem() == net.minecraft.init.Items.leather_chestplate;
            int leatherColor = isLeather ? ((net.minecraft.item.ItemArmor) chestStack.getItem()).getColor(chestStack) : 0xFFFFFF;
            if (isLeather) {
        ...
        if (useArmorTex && chestStack != null && chestStack.isItemEnchanted()) {
    ...
    private com.wildfire.api.IGenderArmor getArmorForStack(ItemStack stack) {
        if (stack == null || !(stack.getItem() instanceof ItemArmor)) return new com.wildfire.api.IGenderArmor() {};
        if (stack.getItem() == net.minecraft.init.Items.leather_chestplate) return new com.wildfire.api.IGenderArmor() {
            @Override public float physicsResistance() { return 0.3f; }
            @Override public float tightness() { return 0.5f; }
        };
        if (stack.getItem() == net.minecraft.init.Items.chainmail_chestplate) return new com.wildfire.api.IGenderArmor() {
            @Override public float physicsResistance() { return 0.5f; }
            @Override public float tightness() { return 0.2f; }
        };
        if (stack.getItem() == net.minecraft.init.Items.golden_chestplate) return new com.wildfire.api.IGenderArmor() {
            @Override public float physicsResistance() { return 0.85f; }
        };
        if (stack.getItem() == net.minecraft.init.Items.iron_chestplate) return new com.wildfire.api.IGenderArmor() {
            @Override public float physicsResistance() { return 1.0f; }
        };
        if (stack.getItem() == net.minecraft.init.Items.diamond_chestplate) return new com.wildfire.api.IGenderArmor() {
            @Override public float physicsResistance() { return 1.0f; }
        };
        return new com.wildfire.api.IGenderArmor() {
            @Override public float physicsResistance() { return 0.5f; }
        };
    }
    ...
    private boolean shouldRenderBreasts(AbstractClientPlayer player) {
        ...
        try { isFake = player.getEntityData().getBoolean("WFG_FakeGUIPlayer"); } catch (Throwable ignored) {}
        if (isFake) {
            Configuration.PlayerGenderSettings fakeCfg = Configuration.getStaticFakeCreditsSettings();
        ...
        Configuration.PlayerGenderSettings settings = Configuration.getPlayerSettings((EntityPlayer) player);
        if (settings == null) {
            try {
                com.wildfire.main.entitydata.EntityConfig ec = com.wildfire.main.entitydata.EntityConfig.getEntity(player);
                if (ec != null) return ec.getBreasts() != null && !"Male".equals(ec.getGender());
        ...
        com.wildfire.api.IGenderArmor armor = new com.wildfire.api.IGenderArmor() {};
        boolean hasChestplate = false;
